...
struct DashboardView: View {
            ...
            ZStack {
                ...
                ScrollView {
                    VStack(spacing: 24) {
                        ...
                        // MARK: – 🧠 Feedback row (显示到下一次新评估开始前)
    ...
    /// 🧠 置信度徽章：颜色根据置信度变化，数值由 Agent sigmoid 公式计算
    ...
    // MARK: – 🧠 Hypothesis-driven Feedback Section

    /// Agent 假设卡片：展示临床推断文本 + 可选反馈选项
    private var feedbackSection: some View {
        VStack(alignment: .leading, spacing: 12) {
            ...
            HStack(spacing: 6) {
                Image(systemName: "brain.head.profile")
                    .foregroundStyle(.purple)
                Text("Agent 分析 · 请确认以下情况")
                    .font(.subheadline.weight(.semibold))
                    .foregroundStyle(.purple)
            }

            // ── 假设文本（Markdown-like 强调部分会自动加粗）─────────
            ...
            if feedbackSubmitted {
                VStack(alignment: .leading, spacing: 4) {
                    Label("模型已更新", systemImage: "checkmark.circle.fill")
                        .font(.caption.bold())
    ...
    private var agentAccuracyPanel: some View {
        VStack(alignment: .leading, spacing: 8) {
            ...
            // 反馈统计 + 公式说明
            HStack {
                Text("反馈 \(viewModel.agent.feedbackCount) 次 · 确认 \(viewModel.agent.confirmedCorrectCount) 次")
                    .font(.caption2)
                    .foregroundStyle(.secondary)
                Spacer()
                // 🧠 显示 EMA 公式，让评审看到算法
    ...
    private var demoButton: some View {
        ...
        .disabled(viewModel.appState != .idle)
        .accessibilityLabel("演示模式：注入模拟预测结果")
    }
    ...
    /// 将 EmaSteps 渲染为评审可读的公式字符串，例如：
    /// "acc: 0.621 × 0.970 + 1.0 × 0.030 = 0.633"
...
// MARK: – SparklineView（精度历史折线图）

/// 迷你 sparkline 折线图，展示 Agent 精度随反馈次数的变化趋势。
