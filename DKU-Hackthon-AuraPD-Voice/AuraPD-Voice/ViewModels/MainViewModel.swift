...
final class MainViewModel: ObservableObject {
    ...
    private func finishCapturing() {
        ...
        // Rule-based 分类仍并行运行，用于 voiceExplanation
    ...
    /// 用户点击 👍/👎 后调用，将正确/错误信号传入 Agent 执行在线学习更新。
    func submitFeedback(correct: Bool) {
        agent.adaptModel(basedOn: correct)
    }

    // MARK: – ⚡ 演示模式：直接注入 Mock 预测，无需采集流程

    /// 绕过 CoreMotion + TTS，用随机生理范围内的 σ 直接生成一条预测结果。
    /// 仅用于黑客松现场演示，让评审可快速连续点击 👍/👎 看到学习曲线变化。
    func injectMockResult() {
        guard appState == .idle else { return }

        // 模拟三种状态的随机 σ（权重：ON 50%、OFF 30%、Tremor 20%）
        let roll = Double.random(in: 0..<1)
        let mockSigma: Double
        if roll < 0.50 {
            mockSigma = Double.random(in: 0.02...0.10)   // ON 区间
        } else if roll < 0.80 {
            mockSigma = Double.random(in: 0.11...0.22)   // OFF 区间
        } else {
            mockSigma = Double.random(in: 0.24...0.40)   // Tremor 区间
        }
        ...
        latestResult = result
        assessmentHistory.insert(result, at: 0)
        statusMessage  = "⚡ Demo 注入完成 — 请点击 👍 / 👎 反馈"
    }
